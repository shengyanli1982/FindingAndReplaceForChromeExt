function GetReplacedText(text, find, findRegex, ignoreCase, replace) {
    ...
    return (
        firstPart.replaceAll(findResult[0], realReplace) +
        GetReplacedText(secondPart, find, findRegex, ignoreCase, replace)
    );
}
...
function DoTaskForElements(rootNode, find, findRegex, ignoreCase, replace, check, highlight) {
    ...
    for (const element of elements) {
        ...
        if (tagName == "input" || tagName == "textarea") {
            ...
            findCount =
                findCount + DoTaskForElements(iframe.body, find, findRegex, ignoreCase, replace, check, highlight);
                ...
                if (node.nodeType === Node.TEXT_NODE) {
                    ...
                    if (replace == result[1] || text.indexOf(result[1]) == -1) {
                        ...
                        if (highlight) {
                            ...
                            tempDiv.innerHTML = text.replaceAll(
                                result[0],
        ...
        if (element.shadowRoot) {
            findCount =
                findCount +
                DoTaskForElements(element.shadowRoot, find, findRegex, ignoreCase, replace, check, highlight);
        }
    }
    return findCount;
}
...
function RemoveHighLightElements() {
    while (true) {
        let hightLightElements = Array.from(document.getElementsByClassName("class_hl_find_and_replace"));
        const iframeElements = document.querySelectorAll("iframe");
        for (const iframeElement of iframeElements) {
            hightLightElements = hightLightElements.concat(
                Array.from(iframeElement.contentDocument.getElementsByClassName("class_hl_find_and_replace"))
            );
        }
